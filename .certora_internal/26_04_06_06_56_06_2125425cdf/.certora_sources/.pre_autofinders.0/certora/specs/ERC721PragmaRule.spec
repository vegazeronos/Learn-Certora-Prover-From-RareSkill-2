...
function helperSoundFnCall(env e, method f){
    if(f.selector == sig:mint(address, uint256).selector){
        ...
        requireInvariant ownerHaveBalances(tokenId);
        burn(tokenId);
    }else if(f.selector == sig:transfeFrom(address, address, uint256).selector){
        address from; address to; uint256 tokenId;
        requireInvariant(tokenId);
        require balanceLimit(to);
        transferFrom(from, to, tokenId);
    }else if(f.selector == sig:safeTransfeFrom(address, address, uint256).selector){
        address from; address to; uint256 tokenId;
        requireInvariant(tokenId);
        require balanceLimit(to);
        safeTransferFrom(from, to, tokenId);
    }else if(f.selector == sig:safeTransfeFrom(address, address, uint256, bytes).selector){
