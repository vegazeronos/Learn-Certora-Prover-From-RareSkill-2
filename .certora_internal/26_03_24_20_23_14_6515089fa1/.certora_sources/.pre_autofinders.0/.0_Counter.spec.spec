...
hook Sstore count mathint counterAfter(mathint counterBefore) {
...
rule check_increment_always_add_one(method f, env e, calldataarg args){
    ...
    //assert
    assert(success <=> counterBefore + 1 == counterAfter, "If and only if success, then counterBefore + 1 == counterAfter");
}