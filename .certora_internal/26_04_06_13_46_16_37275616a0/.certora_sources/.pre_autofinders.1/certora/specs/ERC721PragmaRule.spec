...
rule supplyChange(env e, method f, calldataarg args){
    //precon
    require nonzerosender(e);
    require nonpayable(e);
    requireInvariant zeroAddressHasNoApprovedOperator(e);

    mathint supplyBefore = g_supply;
    //action
    helperSoundFnCall(e, f);
    //poststate
    mathint supplyAfter = g_supply;
...
rule balanceChange(env e, method f, address account){
    //TODO: can only be change throu transfer, burn and mint
    ...
    assert balanceAfter != balanceBefore => (
        ...
        f.selector == sig:safeMint(address,uint256).selector ||
        f.selector == sig:safeMint(address,uint256,bytes).selector ||
        f.selector == sig:transferFrom(address,uint256).selector ||
        f.selector == sig:safeTransferFrom(address,uint256).selector ||
        f.selector == sig:safeTransferFrom(address,uint256,bytes).selector ||
        f.selector == sig:burn(uint256).selector
    );
}

rule ownershipChange(env e, method f, uint256 tokenId){
    //Only can be happen from burn, mint, transferfrom
    require nonzerosender(e);
    ...
    assert (ownerAfter != 0 && ownerBefore == 0) => (
            f.selector == sig:mint(address,uint256).selector ||
            f.selector == sig:safeMint(address,uint256).selector ||
            f.selector == sig:safeMint(address,uint256,bytes).selector
    );
...
rule approvalChange(env e, method f, uint256 tokenId){
    address approveBefore = unsafeGetApproved(tokenId);
    helperSoundFnCall(e,f);
    address approveAfter = unsafeGetApproved(tokenId);

    assert approveBefore != approveAfter => (f.selector == sig:approve(address,uint256).selector ||
        (
            (
