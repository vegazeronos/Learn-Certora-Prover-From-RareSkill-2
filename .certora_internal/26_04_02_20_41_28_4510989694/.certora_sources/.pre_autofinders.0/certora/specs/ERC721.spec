...
rule burn(env e, uint256 tokenId){
    ...
    require otherTokenId != tokenId;
    require otherOwner != ownerTokenIdBefore;
    ...
    //no side effect
    assert ownerOtherTokenIdBefore == ownerOtherTokenIdAfter;
    assert balanceOtherOwnerBefore == balanceOtherOwnerAfter;
}
...
rule burn_example(env e, uint256 tokenId) {
    ...
    address otherOwnerBefore     = ownerOf(otherTokenId);
    ...
    assert ownerOf(otherTokenId)     != otherOwnerBefore     => otherTokenId == tokenId;
}