...
rule check_increment_always_add_one_examples(method f, env e, calldataarg args){
    //precon
    //action
    increment(e);
    //postcon
    //assert
    assert(ghost_counterAfter == ghost_counterBefore + 1, "If and only if success, then counterBefore + 1 == counterAfter");
}