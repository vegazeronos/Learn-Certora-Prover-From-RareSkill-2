...
// ghost mathint g_totalDeposit{
//     init_state axiom g_totalDeposit == 0;
// }

// hook Sstore balanceOf[KEY address owner] uint256 newBalances(uint256 oldBalances){
//     g_totalDeposit = g_totalDeposit + newBalances - oldBalances;
// }

// hook Sload uint256 balance balanceOf[KEY address owner]{
//     require g_totalDeposit >= balance;
// }
...
//TODO: Verify balanceOf(address) <= totalSupply()
invariant noAccount_exceeds_totalSupply(env e1)
...
invariant totalETH_deposit_equals_or_greater_than_WETH_totalSupply()
    // g_totalDeposit >= to_mathint(totalSupply());
    nativeBalances[currentContract] >= totalSupply(){
        preserved with (env e){
            require e.msg.sender != currentContract;
        }
        preserved withdraw(uint256 amount) with (env e){
            requireInvariant noAccount_exceeds_totalSupply();
        }
    }
